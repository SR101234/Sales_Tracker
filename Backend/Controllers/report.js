...
const get_report = async (req, res) => {
    ...
    `WITH SelectedMonthTransactions AS (
        SELECT 
            ...
            -- SIP Aggregations
            SUM(CASE WHEN UPPER(mode) = 'SIP' AND UPPER(nature) IN ('NEW', 'NEW_SIP', 'PURCHASE') THEN amount ELSE 0 END) AS sip_new,
            SUM(CASE WHEN UPPER(mode) = 'SIP' AND UPPER(nature) = 'RELOGIN' THEN amount ELSE 0 END) AS sip_relogin,
            SUM(CASE WHEN UPPER(mode) = 'SIP' AND UPPER(nature) IN ('CLOSED', 'CLOSED_SIP') THEN amount ELSE 0 END) AS sip_closed,
            SUM(CASE WHEN UPPER(mode) = 'SIP' AND UPPER(nature) = 'REDEMPTION' THEN amount ELSE 0 END) AS sip_redemption,

            -- LUMPSUM Aggregations
            SUM(CASE WHEN UPPER(mode) = 'LUMPSUM' AND UPPER(nature) IN ('NEW', 'NEW_SIP', 'PURCHASE') THEN amount ELSE 0 END) AS ls_new,
            SUM(CASE WHEN UPPER(mode) = 'LUMPSUM' AND UPPER(nature) = 'RELOGIN' THEN amount ELSE 0 END) AS ls_relogin,
            SUM(CASE WHEN UPPER(mode) = 'LUMPSUM' AND UPPER(nature) IN ('CLOSED', 'CLOSED_SIP') THEN amount ELSE 0 END) AS ls_closed,
            SUM(CASE WHEN UPPER(mode) = 'LUMPSUM' AND UPPER(nature) = 'REDEMPTION' THEN amount ELSE 0 END) AS ls_redemption,
            
            -- Standalone Redemptions (When mode is logged as 'REDEMPTION' instead of SIP/LUMPSUM)
            SUM(CASE WHEN UPPER(mode) = 'REDEMPTION' THEN amount ELSE 0 END) AS standalone_redemptions
    ...
    SELECT 
        ...
        -- DEDUCT STANDALONE REDEMPTIONS (Adjust this math based on where you want standalone redemptions to hit)
        COALESCE(t.standalone_redemptions, 0) AS standalone_redemptions
